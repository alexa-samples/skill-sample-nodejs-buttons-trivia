...
if [[ $TARGET == "all" || $TARGET == "lambda" ]]; then
    grep "sourceDir" ./skill.json | cut -d: -f2 |  sed 's/"//g' | sed 's/,//g' | while read -r SOURCE_DIR; do
        ...
        ## when deployin to Lambda, the aws-sdk is not really necessary, so we can save about 6MB
        ## and also make it possible to see/edit the code in the Lambda console
        rm -rf $SOURCE_DIR/node_modules/aws-sdk
    done
    echo "###########################"
fi
